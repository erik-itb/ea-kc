...
- **Backend Framework**: WordPress Plugin Architecture (PHP 7.4+)
- **Frontend**: Vanilla JavaScript (no jQuery dependency), CSS3 with BEM-like naming
- **Build Tools**: Composer for PHP dependencies, PHPStan for static analysis
- **Database**: WordPress MySQL with custom post types and taxonomies
- **Assets**: CSS/JS loaded directly (no compilation required)
- **Quality Tools**: WPCS (WordPress Coding Standards), PHP_CodeSniffer, PHPStan

### Development Team Assignments

| Task Type | Agent | Usage Notes |
|-----------|-------|-------------|
| **WordPress/PHP Backend** | `backend-developer` | Primary agent for plugin architecture, custom post types, meta boxes, admin interfaces, and WordPress hooks/filters |
| **JavaScript Frontend** | `frontend-developer` | AJAX search functionality, icon picker UI, language filtering, interactive components |
| **Code Quality & Review** | `quality-engineer` | Test strategies, edge case detection, WordPress security best practices |
| **Performance Optimization** | `performance-engineer` | Database query optimization, asset loading, Core Web Vitals for WordPress |
| **Technical Documentation** | `technical-writer` | Plugin documentation, API references, setup guides, troubleshooting |
| **CSS Styling** | `frontend-architect` | Responsive design, accessibility compliance, typography system improvements |
| **Security & Architecture** | `backend-architect` | Security patterns, data validation, WordPress security hardening |

### Sample Usage Commands
- **PHP/WordPress Development**: `@backend-developer implement custom post type validation`
- **Frontend Features**: `@frontend-developer enhance search autocomplete with keyboard navigation`
- **Code Quality**: `@quality-engineer design test strategy for bilingual content management`
- **Performance**: `@performance-engineer optimize database queries for article filtering`
- **Documentation**: `@technical-writer create installation guide for the plugin`
